...
export class GitHubService {
  ...
  get configured() {
    return Boolean(
      this.token && this.owner && this.repo && this.workflow && this.ref,
    );
  }
  ...
  requireConfig() {
    if (!this.configured) {
      throw new AppError(
        ...
        "GitHub Actions control is not configured on the server.",
      );
    }
  }
  ...
  /**
   * Queue the GitHub Actions workflow.
   *
   * mode:
   *   check       -> run the normal TU notice scraper
   *   test-email  -> send a Gmail test email from GitHub Actions
   */
  async triggerWorkflow(mode = "check") {
    const allowedModes = new Set(["check", "test-email"]);

    if (!allowedModes.has(mode)) {
      throw new AppError(
        400,
        "INVALID_WORKFLOW_MODE",
        `Unsupported workflow mode: ${mode}`,
      );
    }
    ...
    return {
      ...
      message:
        mode === "test-email"
          ? `Gmail test workflow ${this.workflow} was queued.`
          : `Workflow ${this.workflow} was queued.`,
    };
  }
  ...
  async readStateFile() {
    const encodedPath = this.statePath
      .split("/")
    ...
    const file = await this.request(
      this.repositoryPath(
        `/contents/${encodedPath}?ref=${encodeURIComponent(this.ref)}`,
      ),
    );
    ...
    if (!file?.content || file.encoding !== "base64") {
      throw new AppError(
        502,
        "INVALID_STATE_FILE",
        "GitHub did not return a valid bot state file.",
      );
    }
    ...
    try {
      ...
      throw new AppError(
        502,
        "INVALID_STATE_FILE",
        "The bot state file contains invalid JSON.",
      );
    }
  }
  ...
  async updateState(mutator, message) {
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      ...
      const next = normalizeState(
        (await mutator(normalizeState(current.state))) ?? current.state,
      );
      ...
      try {
        ...
        await this.request(this.repositoryPath(`/contents/${encodedPath}`), {
          method: "PUT",
    ...
    throw new AppError(
      ...
      "The bot state changed while it was being updated.",
    );
  }

  async getBotEnabled() {
    const variable = await this.request(
      this.repositoryPath("/actions/variables/BOT_ENABLED"),
      {},
      true,
    );
