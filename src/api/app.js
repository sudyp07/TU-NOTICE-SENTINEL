...
export function createApp({
  ...
  // Render forwards the real client IP through one trusted proxy.
  ...
  app.get("/api/notifications", async (_request, response) => {
    response.json({
      notifications: (await adapter.listNotifications()).slice(-200).reverse(),
    });
  });
  ...
  app.post("/api/check", async (_request, response) => {
    ...
    response
      .status(202)
      .json({ accepted: true, message: result.message || "Workflow queued." });
  });
  ...
  app.post("/api/bot/test", async (_request, response) =>
    response.json({ result: await adapter.runTests() }),
  );
  ...
  app.get("/api/github/status", async (_request, response) =>
    response.json(await adapter.github.latestWorkflowStatus()),
  );
  ...
  app.use((error, _request, response, _next) => {
    ...
    response
      .status(status)
      .json({ error: { code, message, details: error.details || null } });
  });
