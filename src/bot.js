...
export async function runBot({
  ...
  try {
    ...
    console.error(`[${now().toISOString()}] [ERROR] Failed to load state: ${error.message}`);
    previous = { seenIds: [], notices: [], status: {} };
  }
  ...
  try {
    ...
    // Dry run - just log notices without sending email or saving state
    if (dryRun) {
    ...
    if (!dryRun) {
      try {
        ...
        previous.status = {
          ...
          online: false,
          bot: 'failed',
          website: /HTTP|fetch|website|network/i.test(error.message) ? 'error' : (previous.status?.website || 'unknown'),
          scraper: 'failed',
          state: 'error',
          gmail: hasGmailConfig(env) ? (previous.status?.gmail || 'unknown') : 'not-configured',
...
export async function main() {
  try {
    ...
    console.log(`[${new Date().toISOString()}] [INFO] Bot completed successfully:`, result);
    process.exitCode = 0;
  } catch (error) {
